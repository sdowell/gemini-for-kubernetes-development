...
* **Rate Limit Backoff**: Neither interval is a ticker. `Run` decides the delay after each cycle (`nextDelay`), which is normally `Interval` and otherwise the remainder of a `ratelimit.Backoff` wait — 1m doubling to 30m per consecutive refused cycle, jittered, and never shorter than a `Retry-After` GitHub supplied (honoured even past the cap; retrying inside a named wait only earns another refusal). The mechanism is plain error propagation rather than a rate limit the scan keeps checking: every function that makes a GitHub call returns its error, so the first refusal ends the cycle by unwinding out of it, and `ScanOnce` returns it. `Run.cycle` is the single place that reports it, via `github.RetryAfter`, which answers on response status and quota headers rather than on error text — so a 403 denying permission is not mistaken for an exhausted quota. Reporting in one place is deliberate: a report at each call site made one refusal look like dozens and drove the wait straight to its ceiling. `ScanOnce` itself does not consult the backoff: it runs a cycle when asked, and `--once` means the operator wants the scan now. What unwinds is narrower than "any error": inside the per-issue loops (`queueTasks`, `adoptCreatedIssues`) only a refusal ends the pass, and an issue that fails for its own reasons is logged and stepped over — otherwise one unreadable issue would starve every issue behind it, every cycle, since nothing about the next cycle makes it readable. A refusal is different because it says nothing about the issue that met it. So `ScanOnce`'s error means "the cycle was cut short", which is exactly what the backoff wants to hear. Listings are the exception and still fail the cycle outright whatever the cause: a truncated candidate set is published as the complete picture of the open issues, so half of one is worse than none.
* **Responsibilities**:
...
* **Rate Limit Backoff**: Neither interval is a ticker. `Run` decides the delay after each cycle (`nextDelay`), which is normally `Interval` and otherwise the remainder of a `ratelimit.Backoff` wait — 1m doubling to 30m per consecutive refused cycle, jittered, and never shorter than a `Retry-After` GitHub supplied. The mechanism is plain error propagation: every function that makes a GitHub call returns its error, so the first refusal ends the cycle by unwinding out of it, and `ScanOnce` returns it for `Run.cycle` to report exactly once. That is what forced the error-swallowing out of the leaf functions — `evaluateChecks` returning an empty analysis on a failed listing is indistinguishable from a green build, and `CommentState` returning a zero state on a failed fetch is indistinguishable from an unacknowledged comment. Both now return errors. Two consequences follow from this scanner being the expensive one: a refusal ends the pass rather than spending a dozen doomed requests on each pull request left; and a sweep that did not finish does not count as a sweep, since leaving it recorded would spend the next five minutes on fast passes over a candidate set the failure truncated. Only a refusal does this. A candidate that fails for its own reasons — deleted between the listing and the read, checks answering 422 — is logged and skipped, because ending the pass on it would starve every candidate behind it on every cycle, and because withdrawing the sweep claim over it would re-sweep the repository every minute forever. `ScanOnce`'s error therefore means "the cycle was cut short", which is both what the backoff should hear and what the sweep bookkeeping should act on. Because the worker pool evaluates candidates concurrently, the first refusal wins and the feeder stops handing out work — the pool is not cancelled, which would abort the label writes and comments the other workers have in flight.
