...
flowchart TB
    ...
    subgraph Controllers["Autonomous Subcontroller Goroutines"]
        ISC["Issue Scanner<br/>(Single Cycle: 2m<br/>+ 15m sweep)"]
        PSC["PR Scanner<br/>(Single Cycle: 5m<br/>+ 30m sweep, serial)"]
...
* **Cadence**: Two intervals, split along the same line as the other subcontrollers — what is cheap runs often, what paginates does not. Both are measured from the **end** of a cycle rather than its start: a cycle that ran long is one that spent a lot of GitHub requests, usually because it was being throttled, and a fixed-rate ticker would answer that by firing the next cycle the moment the slow one returned.
  * `Interval` (default **2m**) scans the issues that are the watcher's business by ownership: those assigned to a bot in the pool, and those the operator filed themselves. Each is a single page sorted by update time, which is what makes it cheap enough to run at this cadence, and it is what sets pickup latency for a new issue.
  * `SweepInterval` (default **15m**) paginates over every trigger-labelled issue and publishes the open-issue set to `EntityStateCache`. This is the expensive half; running it on the fast ticker would multiply its GitHub traffic tenfold to discover the same issues the fast queries already surface while they are being worked on. It also fires on the first cycle, so a restart does not wait out a full sweep interval.
  ...
  * Both defaults were raised (from 30s / 5m) after the kcc deployment ran into GitHub's hourly rate limit. Pickup latency was never the binding constraint: a throttled scanner picks nothing up at all.
* **Responsibilities**:
...
* **Cadence**: Two intervals rather than the single ticker this document originally specified, for the same reason the reconciler, the chore scheduler and the issue scanner ended up with two. Both are measured from the **end** of a cycle rather than its start, which matters more here than anywhere else: a sweep over a large repository can itself take minutes, and dating the next one from the start of the last would let it begin almost immediately behind a slow one.
  * **5m fast pass** over the pull requests currently assigned to the bot pool — one single-page query per bot account. Assignment is how a pull request is claimed, so this is exactly the set with work in flight, and therefore the set whose CI results, review feedback and merge state change between sweeps.
  * **30m sweep** over every pull request the watcher is responsible for (assigned *or* trigger-labelled, both fully paginated), which also refreshes the open PR half of `EntityStateCache`.
  * The two are **alternatives within a cycle**, not additions: the sweep's candidate set is a superset of the fast pass's, so running both would evaluate the assigned pull requests twice. A single fast ticker over the full set was rejected because evaluating one pull request costs the better part of a dozen GitHub requests; at that cadence a busy repository would exhaust its hourly rate limit.
  * Both defaults were raised (from 1m / 5m) after the kcc deployment ran into that limit in practice.
* **Change detection**: the fast pass evaluates only the candidates that could have moved, which is what stops a cadence measured in minutes from costing a dozen requests per open pull request per cycle. `needsEvaluation` compares the listing's `updated_at` against the one recorded by the last *complete* evaluation, and the sweep never consults it — that asymmetry is the design, because the gate has two blind spots and the sweep is what bounds them:
  * **CI is invisible in `updated_at`.** A check run completing, or being re-run, does not touch the pull request's timestamp. A pull request whose last evaluation saw pending or failing checks is therefore never skipped, which keeps the set the fast pass exists for on every cycle.
  * **So is the queue.** A task finishing is what makes a pull request ready for a human, and it may finish without pushing anything. The gate re-evaluates whenever `HasActivePRTask` disagrees with what the last evaluation recorded.
  * The record lives in the in-package `stateStore` and is deliberately *not* recovered from the queue on startup: a fresh process cannot vouch for a verdict reached before it started, so every pull request earns one full evaluation after a restart.
* **Concurrency**: None — candidates are evaluated one at a time. This reverses the bounded worker pool the design originally called for. The pool did what it was meant to, keeping a pull request whose comment history takes ten seconds to page from delaying the rest, but it only compressed a cycle's requests into a shorter window rather than reducing them, and on a repository with many open pull requests that burst is what exhausted the rate limit. Throttling makes *every* pull request slow, which is strictly worse than the head-of-line delay the pool avoided. Candidates are still deduplicated by number, now so that a pull request that is both assigned and labelled is not evaluated twice in one cycle.
* **Cost of one evaluation**: roughly seven REST requests plus one GraphQL, and the shape matters as much as the number — it must not grow with the pull request's history. Two terms that did were removed:
  * The inline review comments are read with the pull request-wide endpoint and grouped by `PullRequestReviewID` locally, rather than with the per-review endpoint, which cost one request *per review* and so grew without bound on a long-lived change.
  * The issues a pull request closes are resolved through a single per-evaluation `refIssues` fetcher, shared by the label sync, the review opt-in check and the review prompt. Each of those used to fetch them itself, within milliseconds of the others, for the same answer.
* **Responsibilities**:
  ...
  * Evaluates PR state across four **ordered** phases. The order is the design, not an implementation detail: unaddressed feedback wins over everything, because a human asking for a change makes the conflict or CI failure the agent would otherwise chase irrelevant.
    1. **Review Feedback & Comments**: unaddressed human comments and bot reviews (`pr-comments`), acknowledged with `eyes` reactions.
...
| :--- | :--- | :--- |
| **New Issue Pickup Latency** | minutes - hours (sequential loop blocking) | **Up to ~2 minutes** (dedicated single-cycle issue scanner; raised from 30–60s to stay inside the GitHub rate limit) |
...
| **PR Evaluation Concurrency** | Serial loop (all PRs block one another) | **Serial within the scanner, parallel to everything else** (the bounded worker pool was reverted: its request burst, not its latency, was the binding constraint) |
...
* **Verification Gate**:
  * Each step must keep `go test -race ./factory/...` green. Tests move with the code they cover and are rewritten to drive the new constructors rather than a hand-built `Watcher`. The pull request scanner also gained cycle-level coverage for what is new in its step: the sweep/fast-pass alternation, pausing before any request is spent while draining, a clean stop on cancellation, and the candidate dedupe (originally there for the worker pool, and kept so a cycle does not pay for the same pull request twice).
